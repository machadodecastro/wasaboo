...
from _usuarios.forms import RegistrarUsuarioForm
from _usuarios.models import User
from perfis.forms import AvatarForm
from perfis.models import Perfil, Avatar, Follow
from wasaboo import settings
from wasaboo.settings import DEFAULT_FROM_EMAIL
...
class RegistrarUsuarioView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            user = User.objects.create_user(dados_form['email'].lower(), dados_form['senha'])
            user.is_active = False
            
            perfil = Perfil(nome=dados_form['nome'], 
                            email=dados_form['email'].lower(), 
                            type=dados_form['type'],
                            usuario=user)                
            perfil.save()
                       
            current_site = get_current_site(request)
            mail_subject = 'Activate your Wasaboo account'
            ...
            to_email =  dados_form['email'].lower()
            email = EmailMessage(
                        mail_subject, message, to=[to_email]
            )
            email.send()
            return render(request, self.template_name, {'confirm_message': 'Please check your email to activate account'})
